...
{
    public partial class FrPendapatanDatang : Form
    {
        #region Declaration
        ...
        {
            string query = "SELECT TRX_PENDAPATAN_DATANG.ID_PENDAPATAN, CONVERT(VARCHAR(5), ADM_JADWAL.JAM) "+
                      "+ '-' + REF_KOTA.KODE_KOTA + '-' + REF_KOTA_1.KODE_KOTA + '-' + REF_KENDARAAN.NO_ARMADA AS KODE, TRX_PENDAPATAN_DATANG.PENDAPATAN "+
                      "FROM ADM_JADWAL INNER JOIN "+
                      "REF_KENDARAAN ON ADM_JADWAL.ID_KENDARAAN = REF_KENDARAAN.ID_KENDARAAN INNER JOIN "+
                      "REF_KOTA ON ADM_JADWAL.KOTA_ASAL = REF_KOTA.ID_KOTA INNER JOIN "+
                      "REF_KOTA AS REF_KOTA_1 ON ADM_JADWAL.KOTA_TUJUAN = REF_KOTA_1.ID_KOTA INNER JOIN "+
                      "TRX_PENDAPATAN_DATANG ON ADM_JADWAL.ID_JADWAL = TRX_PENDAPATAN_DATANG.ID_JADWAL "+
                      "WHERE ADM_JADWAL.ID_JADWAL = " + cbJadwal.SelectedValue;
            ...
            var form = (FrPendapatanDatang)Application.OpenForms["FrPendapatanDatang"];
            var dgv = (DataGridView)form.Controls["pnlMiddle"].Controls["dgv"];
            dgv.DataSource = table;

            dgv.Columns[0].Visible = false;
            dgv.Columns[1].HeaderText = "Kode Jadwal";
            dgv.Columns[2].HeaderText = "Pendapatan";
        ...
        {
            var dtg = new FrPendapatanDatangAdd(0,Convert.ToInt32(txtIdJadwal.Text),cbJadwal.Text);
            dtg.FormClosed += HandleFormClosed;
            dtg.ShowDialog();
        }
        ...
        {
            var dtg = new FrPendapatanDatangAdd(SelectedId, Convert.ToInt32(txtIdJadwal.Text), cbJadwal.Text);
            dtg.FormClosed += HandleFormClosed;
            dtg.ShowDialog();
        }
        ...
        private void btnHapus_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Apakah anda yakin akan menghapus data ini?", "Konfirmasi", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    string query = "";
                    var con = new SqlConnection(DbConnection.GetConnection());
                    query = "DELETE FROM TRX_PENDAPATAN_DATANG WHERE ID_PENDAPATAN = @Id";
        ...
        private void FrPendapatanDatang_Load(object sender, EventArgs e)
        {
